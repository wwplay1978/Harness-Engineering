...
> 生成说明（渲染/人工填写完成后删除本段；**渲染顺序：先整段删除本说明，再替换正文 span**——本段内「」为规则自指、不属待替换内容）：本模板由安装助手在安装 S5 按**问答渲染**生成为项目根 AGENTS.md（docs/17 §3 S5）。渲染规则=**先删本段，再对正文「」span 整体替换**（span 内已附默认值，替换后安装期指引零残留）：标题 1 处=项目名；快照 5 条按问答答案替换、未提供项用 span 内默认；文档地图 1 处写入本机规范仓 docs 实际绝对路径；**「agent 名」3 处=安装 agent 名称大写连写（如 ZCODE/KIMI），三处同值替换、安装助手自知无须问人**；另有 `<项目名>` 角标 1 处（知识库行）同样以项目名答案替换。知识库行三区号 10/20/30 为**新装默认**——S5 已主动询问是否调整，人调整则该行区号一并按答案替换（agent 段编号恒为 10）。首次落位属**安装窗口一次性豁免**（"AGENTS.md 只能人改"红线网开一面，人复核渲染全文即视为人授权）；落位后红线恢复——此后只能人修改（guard hook 会阻断 Agent 改写）。保持索引式——事实本体放 docs/ 与知识库，这里只放"去哪找"。模板侧维护标记（随本段一并删除，永不落入渲染产物）：constitution-affecting-baseline: 2026-09-06——仅当模板变更**影响已渲染宪法的实质内容**时更新此日期（渲染机制类与新装默认类改动不触发）；sync-harness 项目盘点以它与各项目 AGENTS.md 定稿日比对。
...
- 测试命令：「改成如 uv run pytest -q；默认同构建命令」
...
- 知识库（Obsidian MyObsidian）：定稿知识在 10-知识库/10-「agent 名」/（通用）与
  20-项目库/10-「agent 名」/<项目名>/（项目），进行中未整理知识一律在
  30-工作区/10-「agent 名」/（按类型：工单沉淀/决策速记/环境怪癖/调研笔记）。
  planner 开工前必检两处；**信任分级：定稿区可直接引用，
  工作区（folder 标签 30-*）为未整理草稿、结论须验证**；archiver 只写工作区，定稿区只能人改
...
- 规范总纲与记忆/评估/约束设计：harness 规范仓库 `docs/`：「渲染时写入本机规范仓 docs 绝对路径——标准位 `~/.agents/Harness-Engineering/docs`（Windows 展开 `%USERPROFILE%\.agents\Harness-Engineering\docs`，全机 AI agent 共用一份）；非标准落位机写实际 clone 路径」
