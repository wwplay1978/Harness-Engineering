...
> 生成说明（渲染/人工填写完成后删除本段）：本模板由安装助手在安装 S5 按**问答渲染**生成为项目根 AGENTS.md（docs/17 §3 S5）。渲染规则=**「」span 整体替换**（span 内已附默认值，替换后安装期指引零残留）：快照六项按问答答案替换、未提供项用 span 内默认；文档地图 span 写入本机规范仓 docs 实际绝对路径。首次落位属**安装窗口一次性豁免**（"AGENTS.md 只能人改"红线网开一面，人复核渲染全文即视为人授权）；落位后红线恢复——此后只能人修改（guard hook 会阻断 Agent 改写）。保持索引式——事实本体放 docs/ 与知识库，这里只放"去哪找"。
