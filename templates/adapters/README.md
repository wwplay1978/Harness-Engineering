...
2. **guard 三形态同源**：claude-code / kimi-code / codex 复用 `templates/hooks/guard-worktree.mjs`（stdin JSON → exit 2；部署位=各宿主用户域 hooks 目录 `~/.<宿主>/hooks/harness/`，2026-09-07 起不指向仓库目录）；opencode = 插件 throw；pi = 扩展 `block: true`。白名单（docs//memory//context.md/templates/）、root+'/' 判定、fail-open 语义逐字保持；**自防御条款（SHIELD 表，homedir() 运行时计算）三形态同步携带**。
