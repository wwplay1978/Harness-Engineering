...
- ⚠️ 上游已知问题（记录，不阻塞）：**config.toml `[agents]` 表声明式角色**对 spawn_agent 不可见（#14579）——本包不用该形态（一角色一文件放 `~/.codex/agents/`，项目级 `.codex/agents/` 官方支持、限受信项目）；spawn_agent schema 暴露跨平台不一致（#26828）→ 异常时走 prompts 角色卡降级。
