...
5. **归档是一等公民**。每票收尾必做：合并结论入记忆、Obsidian 知识草稿、度量（含 ±行数）、分支清理——喂给下一票的知识飞轮。
...
- **契约级宿主无关**。治理内核是"约定 + 工件"（宪法、角色纪律、文档树、git 流程）——任何具备派发/子代理/可阻断 hooks/MCP/技能五项机制的宿主都能承载。ZCode 是首个完整实证的适配；Claude Code / Kimi Code 按文档化的四步适配纪律接入（[`docs/16 §2`](docs/16-host-agnostic-installer.md)）。
