...
| 写入隔离 hook | `guard-worktree.mjs` 挂 PreToolUse（Kimi config.toml 注册） | 同脚本挂 ZCode `PreToolUse`，注册进**项目级** `.zcode/config.json` → `hooks.events`，且必须 `hooks.enabled: true` | ① payload schema 可能不同（Kimi 实测 `tool_input.path`，ZCode 待探测，**P0-3**）；② matcher 必须覆盖 ApplyPatch 调用——ZCode 官方文档的别名方向为 `Write/Edit ← ApplyPatch`（ApplyPatch 调用按 Write/Edit 名匹配，理论上 `Write\|Edit` 已覆盖），**加写 `\|ApplyPatch` 为防御冗余，以 P0-3 实测为准**；③ exit 2 阻断语义 ZCode 官方文档确认支持 |
...
| hook 注册位置 | 用户级 `~/.kimi-code/config.toml` | 项目级 `.zcode/config.json`（随 git 走、天然"非本项目静默放行"，比 Kimi 的全局注册+白名单更干净）；**代价：威胁模型变化——注册文件在 Agent 可写范围内，故 guard 白名单必须排除 `/.zcode/`（详见 05 §3）** | 模板见 `templates/zcode-config-template.json` |
