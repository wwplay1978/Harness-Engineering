...
（2026-09-08 分区重编号：原 30-知识库→10、50-项目库→20、70-工作区→30；已有 vault 按旧编号的，重排完成前以磁盘实际目录为准）
```
...
| 分区 | 目录 | 性质 | 谁写入 |
|---|---|---|---|
| **定稿区** | `10-知识库/10-<agent 名>/`（通用知识）、`20-项目库/10-<agent 名>/<项目名>/`（项目知识）——agent 段统一编号 10、段名=AI agent 名称大写连写（如 10-ZCODE；2026-09-08 分区重编号定案） | 整理定稿、高可信 | 只有人（整理归位时） |
| **工作区** | `30-工作区/10-<agent 名>/`（本机=30-工作区/10-ZCODE；按类型分子目录，见下） | 进行中、未整理、草稿 | archiver 起草 + 人随手记 |
...
hindsight-obsidian-sync reconcile \
  ...
  --include "10-知识库" --include "20-项目库" --include "30-工作区/10-ZCODE"
```
（`--vault` 改为你本机 Obsidian 库的实际路径，正斜杠写法；`--include` 为可重复单值，逗号形态无效；include 路径以 vault 磁盘实际分区为准——2026-09-08 分区重编号后，旧编号 vault 重排完成前旧目录名仍可能并存。）
