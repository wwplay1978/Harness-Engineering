...
ZCode 会话
 ...
 ├─ bank：knowledge          ← Obsidian 定稿区(10/20) + 工作区(30-工作区/10-<agent 名>) 单向同步（第 4 节）
...
### 4.0 v3 口径声明（2026-09-10，spec 19 r9）

**全局单一分区口径：50-项目库 / 60-知识库 / 70-工作区**（agent 段=`10-<agent 名>`，项目段=`50-项目库/10-ZCODE/<项目名>/`；避开 vault 既有 10-Outbox/20-日记 占号，撞号遗留关闭）——本节含历史编号（10/20/30）的叙述按写作时口径理解，**会被直接执行的命令与现行语义表以本声明后的 v3 口径为准**；分区名/agent 段/项目段运行时以两级 harness 配置为准（机器级 `~/.agents/Harness-Configuration/`，缺失时 check-env 回退内置默认 50/60/70 并 WARN）。在装项目按 spec 19 §6 四阶段手册升级切换。
...
（2026-09-08 重编号：原 30-知识库→10、50-项目库→20、70-工作区→30；原 10-Outbox / 20-日记 与新号冲突，待 vault 侧重排后更新本行）
```
...
| 分区 | 目录 | 性质 | 谁写入 |
|---|---|---|---|
| **定稿区** | `60-知识库/10-<agent 名>/`（通用知识）、`50-项目库/10-<agent 名>/<项目名>/`（项目知识）——agent 段编号 10、段名=AI agent 名称大写连写（如 10-ZCODE；v3 全局口径 50/60/70 替代 2026-09-08 的 10/20/30，历史沿革见 4.0 声明） | 整理定稿、高可信 | 只有人（整理归位时） |
| **工作区** | `70-工作区/10-<agent 名>/`（本机=70-工作区/10-ZCODE；按类型分子目录，见下） | 进行中、未整理、草稿 | archiver 起草 + 人随手记 |
...
（include 路径以 vault 磁盘实际分区为准——2026-09-08 分区重编号后，vault 侧重排完成前旧目录名仍可能并存。）
...
- 入库范围：定稿区（10/20）+ 工作区（30-工作区/10-ZCODE）都入库——工作区知识是最新鲜的，planner 开工就要能检到；**信任分级靠 folder 标签**（定稿区结论可直接引用，工作区内容需注明"未整理"并以定稿区为准）。排除 00-Inbox/Outbox/日记/90-附件（隐私与噪声——分区重编号后以 vault 实际目录为准）。
...
1. **工单级（每单）**：人合并后 archiver 把"本单可沉淀知识"写成草稿 →
   `30-工作区/10-ZCODE/工单沉淀/<slug>.md`（文件头含来源工单、spec 修订号、合并 hash，可追溯）→
   随即触发一次 reconcile（archiver SOP 第 4 步），当单即可被 recall。
2. **整理级（人主导，周或里程碑节奏）**：人（可派 docs-architect 协助）把工作区草稿去重、提炼、归位到
   `10-知识库/10-<agent 名>/`（通用）或 `20-项目库/10-<agent 名>/<项目名>/`（项目特定），原工作区笔记删除（同步自动修剪 bank）——工作区保持"只放未整理"语义。首次全量实转：2026-09-06 web2api 五票沉淀归位（工单索引+环境怪癖两件入定稿区、工作区清零）。

飞轮由此闭合：**工单 → 工作区草稿（即时可检索）→ 人工整理定稿 → 下个工单的 recall 注入**。

### 4.4 项目库页面约定（20-项目库/10-<agent 名>/<项目名>/，定稿区）

每个启用本体系的项目一页（或一目录）：`20-项目库/10-<agent 名>/<项目名>/`——架构决策记录（ADR 风格）、环境怪癖定稿、与 basic-memory decisions/ 的互相引用。planner 开工前必检**两处内容源**：定稿区本项目页（定稿）+ 30-工作区/10-ZCODE/ 近期未整理笔记（最新鲜）；访问通道两条：Obsidian 直读或 knowledge bank 检索（planner.md 行为校准列的第三处即 knowledge bank 通道，口径一致）。
