...
This repository is a host-agnostic harness specification + installer toolkit: a six-role agent pipeline (planner → developer → code-reviewer → qa-tester → red-teamer → archiver), physical write-isolation guardrails, three-layer memory (basic-memory / hindsight / Obsidian), an executable audit spec, and a three-phase installer (detect → install → adapt) that an AI agent can drive end-to-end. A human handles elevation, GUI installs, credentials, and decisions.

> **Core philosophy: humans steer, agents execute.**

> Validated in a real pilot: 6 tickets end-to-end, 0 implementation defects found in QA, red-team hits with zero overlap vs. review+QA, independent harness audit baseline 93/100 (A).

---

## Why Harness Engineering?

AI coding works great for an individual. It breaks the moment a team, a second machine, or a new quarter enters the picture:
...
- Quality is whatever the model happens to produce that day. Model upgrades silently change behavior, and no gate defines what counts as done.
...
Harness Engineering's answer, echoing OpenAI's agent-first field report (see [References](#references)), is to treat the AI as an engineering unit that rules can constrain and measurement can improve. Wrap it in a harness you own:

- you can't control the model, but you can control the harness;
...
Six roles, one ticket, from spec to archive (full detail in [`docs/02`](docs/02-team-core.md)):

[![Six-role pipeline](docs/diagrams/pipeline-light.png)](https://pages.20081005.xyz/pipeline.html)

The animated diagram is [live-hosted here](https://pages.20081005.xyz/pipeline.html); click the image above to open it. It plays a live trace of one ticket's journey through all six roles, with Light/Dark toggle, pan/zoom, search, and guided chapters. The same self-contained files ship in this repo under [`docs/diagrams/`](docs/diagrams/) if you prefer to open them locally.
...
What changed: all per-project and per-machine parameters moved out of the AGENTS.md constitution into a cascading config center (machine-level `~/.agents/Harness-Configuration/` = shared `common.config.md` + per-host `<host>.config.md`; project-level `harness.config.md` at project root) ([`docs/19`](docs/19-constitution-config-split.md)). The constitution is now zero-parameter and byte-identical across all projects and hosts: installed by plain copy, drift-checked byte-for-byte by `sync-harness`. Zone defaults are unified to `50-Projects / 60-Knowledge / 70-Workspace` with agent segment `10-<AGENT>`.

The merge gate was revised (user-ratified): quality gates green means auto-merge with trace (`pre-merge-check.mjs` assertions + an `auto-merge: gates green` footnote); any red goes to a human ([`docs/20`](docs/20-autonomous-merge.md)). A spec-level red-team gate now runs between planner and development, with the planner working staged A→B→C: spec + red-team grading (A), P0/P1 closure with disputed findings kept blocked for human adjudication (B), tickets only after human final confirmation (C). The archiver fires on both auto-merge and human merges. Amendment 2026-09-14 (D6): the constitution template gained a parallel-memory prohibition. Memory flows only through the three sanctioned channels (git/basic-memory · hindsight · Obsidian); private per-agent memory trees are banned in harness-governed projects.
...
Interactive version: [live-hosted here](https://pages.20081005.xyz/architecture.html). It covers the human contract, host layer, six-role pipeline, physical write isolation, four-layer memory, and the spec feedback loop, with theme toggle, focus/search, and guided chapters.

```
Harness-Engineering/
├── docs/        reusable spec layer (concepts, six roles, memory, eval, guardrails,
│                host adapters, constitution–config split, autonomous merge,
...
│   ├── adapters/          ready-to-use packages for the 5 adapted hosts (docs/18)
...
│   ├── tools/             sync-harness v2 distributor (byte-identity + config
│   │                      schema) · pre-merge-check merge-gate assertions ·
│   │                      role-variant generator
│   ├── skills/            harness-audit executable spec
│   ├── AGENTS-template.md zero-parameter constitution skeleton (incl. commit whitelist)
│   ├── harness-*-template.md  cascading config center templates (machine-level
│   │                      common/host + project-level harness.config.md)
...
- **Host-agnostic by contract.** The governance core is conventions + artifacts (constitution, role discipline, docs tree, git flow); any agent host providing dispatch, subagents, blocking hooks, MCP and skills can carry it. ZCode is the first fully validated adapter. The mainstream hosts (Claude Code, Kimi Code, Codex, OpenCode, Pi Agent) follow a documented 4-step adaptation discipline ([`docs/16 §2`](docs/16-host-agnostic-installer.md)); ready-to-use adapter packages for all five ship in [`templates/adapters/`](templates/adapters/) ([`docs/18`](docs/18-host-adapters.md)), with on-machine installation checks left as the first-install step.
...
Choose by need: for planning discipline with a huge community, spec-kit; for a full agile process, BMAD; for Claude Code superpowers, SuperClaude; for task graphs, Task Master. What this repo adds on top: enforcement that is physical rather than advisory, an adversarial gate before merge, archive as a first-class knowledge flywheel, and a measurable audit loop, all host-agnostic by contract.

## Measurement & continuous improvement

- **Metrics ledger per ticket** (`docs/04`): attempts, review rounds, QA rounds, QA defects, first-pass rate, test count, ±lines, recorded by the archiver and reviewed on a rolling cadence.
- **`harness-audit`**: an executable 7-dimension, 100-point audit spec (constitution quality, rules & enforcement, skills layering, tools, SDD process, engineering gates, memory & archive) producing S/A/B/C/D with P0–P3 remediation, run quarterly and after major spec changes. Pilot baseline: 93/A.
- **Quality gates that bite**: first-pass ≠ 100% in early tickets is healthy. It means reviewer/QA gates catch real issues; severity trends should decay over time (P1 → P2/P3 in the pilot).
...
- **Knowledge flywheel**: ticket → workspace draft (instantly searchable via vault→bank reconcile) → human curation into the final zone → next ticket's recall. The three memory layers close the loop.
